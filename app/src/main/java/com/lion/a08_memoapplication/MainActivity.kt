...
import com.lion.a08_memoapplication.fragment.SearchMemoFragment
...
//            subMenuItem?.setIcon(R.drawable.lock_24px)
            ...
            navigationViewMain.setNavigationItemSelectedListener {
                when(it.itemId){
                    ...
                    R.id.navigation_menu_item_search -> {
                        replaceFragment(FragmentName.SEARCH_MEMO_FRAGMENT, false, false, null)
                    }
                    else -> {
                        val dataBundle = Bundle()
                        // 메모 목록 구분 값
    ...
    fun replaceFragment(fragmentName: FragmentName, isAddToBackStack:Boolean, animate:Boolean, dataBundle: Bundle?){
        ...
        val newFragment = when(fragmentName){
            ...
            // 검색 화면
            FragmentName.SEARCH_MEMO_FRAGMENT -> SearchMemoFragment()
        }
